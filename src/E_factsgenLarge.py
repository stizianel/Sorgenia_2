...
lfacts=[("select legacy, tipo, operand, saison, prog from pemg_facts_quant a where a.legacy = :key group by legacy, tipo, operand, saison, prog", "select legacy, tipo, ab, bis, menge, tarifart, kondigr  from pemg_value_quant  a  where a.legacy = :key and a.prog = :prog"),
